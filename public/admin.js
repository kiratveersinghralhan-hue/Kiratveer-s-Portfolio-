(() => {
  ...
  if ((content.version || 5) < 6 && !content.projects.some(item => item.id === "qortra")) content.projects.unshift(clone(defaults.projects.find(item => item.id === "qortra")));
  ...
  function save(message = "Changes saved locally") { content.version = 6; localStorage.setItem(CONTENT_KEY, JSON.stringify(content)); render(); toast(message); }
